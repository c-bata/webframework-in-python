...
DjangoやFlaskを使ったWeb開発に慣れてきた頃、より高度な問題に対処できるよういくつかのWebフレームワークの実装を読み始めました。
最初はどこから手をつけて読んでいけばいいかも分かりませんでしたが、時間をかけながらも手当たり次第にBottleやDjangoの実装を読み進めることで次第にそれらの全体像が見えるようになってきました。
その後さらに理解を深めるために `Kobin <https://github.com/kobinpy/kobin>`_ というフレームワークを作り始めたのですが、フレームワークをデザインする中で、各種フレームワークの設計方針の違いに気づくようになりました。
またフレームワークを利用しているだけでは気づくことのできなかった多くのHTTPに関する知識を得ることができました。
最近ではGo言語などPython以外の言語を書くことも多くなってきましたが、ここで得た多くの知識が役に立っています。
